...
class DotaBot extends Bot {
	...
	_onChannelMessage(message) {
		...
		switch(message.text) {
			...
			case '!stats': {
				...
				say(`${name} has ${stats.points} points`);
				break;
			}
			case '!h': {
				if (!trivia.roundStarted) {
				...
				hint ? say(`hint: \`${hint}\``) : 0;
				break;
			}
			case '!help': {
				say('`!q` to get a question or the current one. `!h` for the current hint. `!stats` for your stats.');
			}
			default: {
				if (trivia.roundStarted) {
					let result = trivia.makeGuess(name, message.text);
					if (result.status === CORRECT) {
						say(`Yay, ${name}! \`${answer}\` is the correct answer. +${result.earned} (total points: ${result.points} ${result.streak > 1 ? `win streak: ${result.streak}` : ""})`);
						this._cleanup();
					}
				}
			}
	...
	_roundControl(channel, message) {
		...
		if (this.timeIntervals >= 4) {
			say(`You guys are terrible at Dota2 :lul: The answer is \`${this.trivia.getAnswer()}\`.`)
			this._cleanup();
		} else if (hint) {
			if (!this.hintTS) {
				say(`hint: \`${hint}\``);
